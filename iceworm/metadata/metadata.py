...
class Object(dc.Enum, allow_setattr=True, reorder=True):
    ...
    aliases: ta.AbstractSet[QualifiedName] = dc.field(
        (), kwonly=True, coerce=col.abs_set, check=lambda l: all(isinstance(o, QualifiedName) for o in l))
...
class Table(Object):
    columns: ta.Sequence[Column] = dc.field(coerce=col.seq, check=lambda l: all(isinstance(o, Column) for o in l))

    def __post_init__(self) -> None:
        self._columns_by_name: ta.Mapping[str, Column] = col.unique_dict((c.name, c) for c in self.columns)
...
class Function(Object):
    ...
    params: ta.Mapping[str, dt.Datatype] = dc.field(
        coerce=col.map, check=lambda d: all(isinstance(k, str) and isinstance(v, dt.Datatype) for k, v in d.items()))


def _build_obj_map(objs: ta.Iterable[ObjectT], cls: ta.Type[ObjectT]) -> ta.Mapping[QualifiedName, ObjectT]:
    return col.unique_dict(
        (n, o)
        for o in objs
...
class Catalog(dc.Frozen, final=True, allow_setattr=True):
    tables: ta.Sequence[Table] = dc.field((), coerce=col.seq, check=lambda l: all(isinstance(o, Table) for o in l))
    functions: ta.Sequence[Function] = dc.field((), coerce=col.seq, check=lambda l: all(isinstance(o, Function) for o in l))  # noqa
